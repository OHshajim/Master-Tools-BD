...
interface BulkDraftCookieTableProps {
  draftPlatforms: DraftPlatformStatus[];
  getPlanName: (planId: string) => string;
  getPlatformName: (platformId: string) => string;
}

export const BulkDraftCookieTable: React.FC<BulkDraftCookieTableProps> = ({
  draftPlatforms,
  getPlanName,
  getPlatformName
}) => {
  ...
  const confirmRestore = () => {
    if (restoringDraft && user) {
      ...
      toast({
        ...
        description: "The cookie platform has been restored to the user's access page."
      });
  ...
  if (draftPlatforms.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        ...
        <p>No drafted cookie platforms found</p>
        <p className="text-sm">All cookie platforms are currently visible to users</p>
      </div>
    );
  }
